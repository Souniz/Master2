...
def get_actions(state,actions,M):
    ...
    for a in actions :
        ...
        if a == "UP" :
            ...
            m[1] = +1
        if M[state[0]+m[0],state[1]+m[1]] == 1.0 :    
...
def generate_episode(init_state,actions,M,exit_state,itermax=1000) :
     ...
     while  i < itermax and not all(current_state == exit_state)  :      
         current_actions = get_actions(current_state,actions,M)
         action = current_actions[np.random.randint(len(current_actions))]
         new_state, reward= next_state(current_state, action, exit_state, M)
         #
